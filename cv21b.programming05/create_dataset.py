    ...
    for i in range(nSamples):
        ...
        if checkValid:
            if not checkImageIsValid(imageBin):
                print('%s is not a val image' % imagePath)
                continue
...
if __name__ == '__main__':
    ...
    # 需要运行 2 次，分别生成 train，val 的数据库 将 125 127 的 train 换成val
    parser.add_argument('--out', type = str, default="lmdb/train", help = 'lmdb data output path')
    parser.add_argument('--folder', type = str, help = 'path to folder which contains the images')
    parser.add_argument('--file', type = str,default="train.txt", help = 'path to file which contains the image path and label')
