...
from data_extract import MyDataSet
...
def create_model(num_classes, device):
    ...
    weights_dict = torch.load("fasterrcnn_resnet50_fpn_coco.pth", map_location=device)
...
def main(parser_data):
    ...
    train_data_set = MyDataSet(data_root, data_transform["train"], "train")
    ...
    val_data_set = MyDataSet(data_root, data_transform["val"], "val")
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录(VOCdevkit)
    parser.add_argument('--data-path', default='cv21b.programming02-dataset', help='dataset')
    # 检测目标类别数(不包含背景)
    parser.add_argument('--num-classes', default=471, type=int, help='num_classes')
    # 文件保存地址
